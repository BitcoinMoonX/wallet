myApp.onPageInit('wallet', function (page) {
            ...
            for(var j = 0; j < addresses.length; j++) {
                if(cacheWallet[j] != null) {
                    ...
                    if(cacheToken != undefined && cacheToken.length > 0) {
                        balanceAccountbtcmUSD = cacheToken[0].balances * localStorage.getItem("bitcoinmoonPrice");
                        document.getElementById("balancebtcm" + j).innerHTML = escapeHtml(format(cacheToken[0].balances) + ' BTCM');
                        document.getElementById("balanceAccountbtcmUSD" + j).innerHTML = "$" + format(balanceAccountbtcmUSD, 2);
                    } else {
                        document.getElementById("balancebtcm" + j).innerHTML = '0 BTCM';
                        document.getElementById("balanceAccountbtcmUSD" + j).innerHTML = "$0";
                    }
        ...
        <li style="width: 100%; border-bottom: 1px solid #ddd;">\
            ...
            <div class="item-inner" style="background-color: none;" id="address'+i+'">\
                ...
                <div class="item-subtitle" style="margin-top: 5px;">\
                    <div style="display: flex; justify-content: space-between; align-items: center;">\
                        <span style="color: #333;" id="balancebtcm'+i+'"></span>\
                        <span style="color: #666;" id="balanceAccountbtcmUSD'+i+'"></span>\
                    </div>\
                </div>\
            </div>\
            </a>\
    ...
    document.getElementById("helpAccounts").addEventListener("click", function () {
       helpPopover("Accounts",'<strong>Accounts</strong> : it is your Ethereum addresses.</br>This address can store Ethers and BTCM Tokens. In order to see the details about an address, click on account name.</br>Account name generated randomly you can edit the account name on next page.', this);
    });
...
//                });
//
       //         localStorage.setItem("customAccounts", parseInt(localStorage.getItem("customAccounts")) + 1);
//
