myApp.onPageInit('address', function (page) {
    ...
    function loadTokens() {
        ...
        if(cacheToken.length === 0) {
            let address = '0x129c8aC258bf01a43f6511EA7951Ff762B34D345';
            let missing = true;
        ...
        if(cacheToken != undefined && cacheToken.length === 1) {
            ...
            document.getElementById("sendbtcm").addEventListener("click", (function (arg1) {
                return function () {
                        ...
                        for(var i = logs.length - 1; i >= 0; i--) {
                            ...
                            if(listTxs[i].timestamp > lastTxTimestamp) {
                                if(currentEvent._eventName == "Transfer" && (currentEvent._to == currentAddress || currentEvent._from == currentAddress)) {       
                                    ...
                                    var amount = '<span style="color:'+color+'">'+plusMinus+''+format(parseFloat(escapeHtml(currentEvent._value) / Math.pow(10, 18))) + ' BTCM</span>';
                                }
    ...
    document.getElementById("helpTokens").addEventListener("click", function () {
       helpPopover("BTCM Tokens / Transfers",'<strong>BTCM Tokens</strong> : shows the balance of BTCM Tokens on this Ethereum address.<br/><br/><strong>Transfers</strong> : it is the list of all the token transfers on this account.', this);
    });
        ...
        <li class="swipeout" style="width:100%">\
            <div class="swipeout-content">\
                <div class="item-content">\
                    ...
                    <div class="item-inner">\
                        <div class="item-title-row">\
                            <div class="item-title">BitcoinMoon</div>\
                        </div>\
